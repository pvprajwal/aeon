...
if __name__ == "__main__":
    ...
    def _parallel_tlb(dataset_name, n_segments, alphabet_size):
        ...
        for histogram, variance in itertools.product(histograms, variances):
            ...
            sfa.fit(X_train)
            Y_words = sfa.transform_words(X_test)
        ...
        for i in range(min(X_train.shape[0], X_test.shape[0])):
            ...
            for j, (sfa, X_dfts, Y_words) in enumerate(sfa_transforms):
                ...
                mindist[j + 1] = dft_sfa_mindist(X_dfts[i], Y_words[i].squeeze(), sfa.breakpoints)
            ...
            for a, mind in enumerate(mindist):
                if mind > ed:
                    print(f"mindist {method_names[a]} is:\t {mind} but ED is: \t {ed}")
            ...
            for a, method_name in enumerate(method_names):
                ...
                sum_scores[method_name]["mindist"].append(mindist[a])
                sum_scores[method_name]["tightness"].append(tighness[a])
    ...
    ).to_csv(f"logs/tlb_all_ucr_{n_segments}_{alphabet_size}-16-10-24.csv", index=None)
