...
def read(fp, dim, data_type=np.float32, count=100):
    if data_type != np.float32:
        ...
        return np.fromfile(fp, dtype=np.float32, count=dim * count).reshape(-1, dim).astype(np.float32, copy=False)
...
def compute_distances(
    ...
    eds = np.zeros(queries.shape[0], dtype=np.float64)
    eds[:] = np.inf
    ...
    for i in prange(queries.shape[0]):
        for j in range(samples.shape[0]):
            ...
            if ed > 0:
                for a, md in enumerate(mindist):
                    tighness[a] += md / ed / samples.shape[0]

            for a, mind in enumerate(mindist):
                if mind > ed:
                    print(f"mindist {method_names[a]} is:\t {mind} but ED is: \t {ed} \t Pos: {i}, {j}")
                    # print(f"Query std/mean: \t {np.std(queries[i])}, {np.mean(queries[i])}")
                    # print(f"Sample std/mean:\t {np.std(samples[j])}, {np.mean(samples[j])}")
...
for dataset in datasets:
    for alphabet_size in alphabet_sizes:
        ...
        samples = read(path + file_data, dim=d, data_type=data_type, count=100_000)  # data
        ...
        SAX_transform = SAX(n_segments=n_segments, alphabet_size=alphabet_size, znormalized=True)
        ...
        tightness, eds = compute_distances(
            queries,
            samples,
            PAA_queries,
            SAX_samples,
        ...
        for name, _ in sum_scores.items():
            print(f"---- Name {name}, \t tlb: {np.round(sum_scores[name]["tightness"], 3)}")
        ...
        pd.DataFrame.from_records(
            ...
            f"logs/tlb_all_sofa_bench_{n_segments}_{alphabet_size}-24-10-24.csv",
            index=None)
